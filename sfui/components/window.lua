...
function Window:render(cursor, action)
    ...
    render.drawRectFast(self.mins.x, self.mins.y, self.size.x, self.size.y * SFUi.aspectRatio)

    render.setColor(self.dragging and self.palette.hover or self.palette.component)
    render.drawRectOutline(self.mins.x, self.mins.y, self.size.x, self.size.y * SFUi.aspectRatio)

    render.setColor(self.dragging and self.palette.hover or self.palette.component)
    render.drawRectFast(self.mins.x, self.mins.y, self.size.x, self.barheight * SFUi.aspectRatio)
    ...
    if self.closehides ~= nil then
        ...
        render.drawRectFast(self.maxs.x - self.barheight, self.mins.y, self.barheight, self.barheight * SFUi.aspectRatio)

        render.setColor((self.extrahover.close and self.action.held) and self.palette.highlight or self.palette.foreground)
        render.drawLine(self.maxs.x - self.barheight * 0.75, self.mins.y + self.barheight * 0.25, self.maxs.x - self.barheight * 0.25, self.mins.y + self.barheight * 0.75 * SFUi.aspectRatio)
        render.drawLine(self.maxs.x - self.barheight * 0.75, self.mins.y + self.barheight * 0.75, self.maxs.x - self.barheight * 0.25, self.mins.y + self.barheight * 0.25 * SFUi.aspectRatio)
    end
