...
function Window:render(cursor, action)
    ...
    if type(self.paint) == "function" then
        ...
        if type(self.override_design) == "function" then
            ...
            render.drawRoundedBoxEx(90, self.mins.x, self.mins.y, self.size.x, self.barheight * self.aspectRatio, true, true, false, false)
            ...
            if self.can_maximise and self.resizable then
                ...
                render.drawRoundedBox(5, self.mins.x + self.size.x - self.barheight*2 + 0.5, self.mins.y + self.aspectRatio / 2, self.barheight - 1, (self.barheight - 1) * self.aspectRatio)

                render.setColor((self.focus.allowed or self.dragging or self.hold_resize) and self.palette.hover or self.palette.component)

                render.drawRoundedBox(5, self.mins.x + self.size.x - self.barheight*2 + 1.5, self.mins.y + 1.5, self.barheight - 3, (self.barheight - 3) * self.aspectRatio)

                render.setColor(Color(255,255,255))

                render.drawRectOutline(self.mins.x + self.size.x - self.barheight*2 + 4, self.mins.y + 5, self.barheight - 7, 3)
                render.drawRectOutline(self.mins.x + self.size.x - self.barheight*2 + 4, self.mins.y + 5, self.barheight - 7, (self.barheight - 8) * self.aspectRatio, 1)
            end
            //
        end
    end
